...
The inbox receives committed handoff summary artifacts from the governance hub repo for each implementation spec. Each file is a self-contained summary that an espresso-logs implementation agent can use to execute the scoped task list without querying `governance hub` directly.
...
3. Confirm no governance hub file path access is needed (the handoff summary should be self-contained)

...
## Coffee Tracker Override

When reviewing Coffee Tracker UI, defer to `docs/requirements/design-language.md` and
`.squad/skills/ui-design-contract/SKILL.md` over generic aesthetic preferences. Screenshots remain
visual evidence, but contract enforcement defaults to Playwright computed-style assertions rather
than screenshot diffing.
...
3. **Capture screenshots of the running application.**
   ...
   1. **Playwright-capable browser automation (preferred).** Check the available MCP/browser tools for a Playwright or equivalent browser automation server. Tool and server names vary by environment, so inspect what is available rather than assuming a specific name. Use the first option that supports viewport sizing, full-page captures, and file naming.
   2. **IDE or in-app browser screenshot tool (second choice).** If Playwright-capable automation is not available, use any available IDE/browser screenshot tool with equivalent navigation and capture capabilities.
